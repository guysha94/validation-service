...
function Plugins({value, onChange, placeholder, floatingAnchorElem, onRef, isInitialMount}: PluginsProps) {
    const [editor] = useLexicalComposerContext()
    const isUserTypingRef = useRef(false)
    const lastEditorValueRef = useRef<string>("")

    // Initialize editor with code node on mount
    useEffect(() => {
        if (isInitialMount.current) {
            editor.update(() => {
                ...
                // Always initialize with code node - replace any existing nodes
                const hasCodeNode = children.some(child => $isCodeNode(child))
                if (!hasCodeNode || children.length === 0) {
                    root.clear()
                    const codeNode = $createCodeNode("sql")
                    if (value) {
                        const textNode = $createTextNode(value)
                        codeNode.append(textNode)
                    } else {
                        // Even if empty, add an empty text node so user can type
    ...
    // Update editor when value prop changes (only from external sources)
    useEffect(() => {
        // Skip update if user is currently typing (to prevent cursor jump)
        if (isUserTypingRef.current || isInitialMount.current) {
            return
        }
        ...
        editor.getEditorState().read(() => {
            ...
            if (children.length === 1) {
                ...
                if ($isCodeNode(firstChild)) {
                    currentText = firstChild.getTextContent()
                }
            }

            // Only update if the value is different from current content
            // and different from what we last set (to avoid unnecessary updates)
            if (currentText !== value && lastEditorValueRef.current !== value) {
                editor.update(() => {
                    const root = $getRoot()
                    root.clear()
                    const codeNode = $createCodeNode("sql")
    ...
    return (
        <div className="relative">
            <ToolbarPlugin>
                {({blockType}) => (
                    <div
                        className="vertical-align-middle sticky top-0 z-10 flex items-center gap-2 overflow-auto border-b p-1 bg-background">
                        ...
                        <div className="">
                            <div className="" ref={onRef}>
                                <ContentEditable
                                    ...
                                    className="ContentEditable__root relative block min-h-[200px] overflow-auto px-4 py-3 focus:outline-none font-mono text-sm"
                                />
                            </div>
                        </div>
                    }
                ...
                <OnChangePlugin
                    ...
                    onChange={(editorState) => {
                        editorState.read(() => {
                            ...
                            // Ensure we have a code node - convert paragraphs to code nodes if needed
                            ...
                            if ($isRangeSelection(selection)) {
                                ...
                                if ($isParagraphNode(node)) {
                                    // Convert paragraph to code node
                                    editor.update(() => {
                                        const currentSelection = $getSelection()
                                        if ($isRangeSelection(currentSelection) && currentSelection.isCollapsed()) {
                                            $setBlocksType(currentSelection, () => $createCodeNode("sql"))
                                        }
                                    }, {discrete: true})
                                }
                            }

                            // Extract text from code node or paragraph
                            let text = ""
                            ...
                            // Update the last known editor value
                            ...
                            // Reset the flag after onChange completes
